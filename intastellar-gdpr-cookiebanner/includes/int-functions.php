...
function initIntastellarAdminStyles($hook)
{
    ...
    $plugin_version = get_plugin_data(__FILE__)['Version'] . time();
...
function initIntastellarSettingsPage()
{
    ...
    add_submenu_page("intastellar-consents", "Consents Platform", "Consents Platform", "manage_options", "intastellar-consents/platform", "intastellarPlatform", null);
...
<?php } ?>
<?php
function intastellarPlatform()
{
    $plugin_version = get_plugin_data(__FILE__)['Version'];
    $domain = get_option('intastellarSiteRoot');
    ...
    {
        $issuedAt = time();
        $sign_key = generateRandomString(256);
        $header = [
            "alg" => "HS512",
            "typ" => "JWT"
        ];
        ...
        $payload = [
            "exp" =>  strtotime("+30 days", $issuedAt),
            "nbf" => $issuedAt,
            "iss" => "Intastellar Solutions, International",
            "PAYLOAD" => $data
        ];

        $payload = base64_url_encode(json_encode($payload));
        $signature = base64_url_encode(hash_hmac("sha512", $header . "." . $payload, $sign_key, true));
    ...
    $token = JWTGenerate($scope);

?>
    <iframe id="intastellar-consents" src="https://www.intastellarconsents.com/gdpr/view/<?php echo urlencode($domain); ?>?token=<?php echo $token; ?>" width="100%" style="height: 90vh;"></iframe>
    <section class="intastellarPluginContent">
        <?php include("intastellarGDPRAdminPanelFooter.php"); ?>
    </section>
<?php
